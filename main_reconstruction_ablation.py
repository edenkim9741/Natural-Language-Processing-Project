...
def tokenize_dataset(all_path, dataset_path):
    # 토크나이저 및 모델 로드
    tokenizer = PreTrainedTokenizerFast.from_pretrained("gogamza/kobart-base-v2")
    model = BartForConditionalGeneration.from_pretrained("gogamza/kobart-base-v2")
    

    dataset = load_dataset("csv", data_files={"full": all_path})["full"]

    def preprocess_function(examples):
        inputs = tokenizer(examples["input"], max_length=256, truncation=True, padding="max_length")
        targets = tokenizer(examples["original_text"], max_length=256, truncation=True, padding="max_length")
    ...
    tokenized_dataset = dataset.map(preprocess_function, batched=True, remove_columns=["input", "original_text", "translated_text"], num_proc=os.cpu_count())
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(all_indices)):
        if args.log:
            ...
            wandb.init(
                ...
                name=datetime.now().strftime(f"reconstruction_ablation_%Y%m%d_%H%M%S_fold{fold}"),
                config={
                    "epochs": args.epochs,
                    "batch_size": args.batch_size,
                    "learning_rate": 5e-5,
                    "model": "PhoneticFusionBART_Reconstruction",
