...
def process_sentence(args):
    ...
    return g2p(sentences_w_idx, sampling_num=3)

def chunkify(lst, n):
    return [lst[i:i+n] for i in range(0, len(lst), n)]

json_files = []
root_dir = "/home/jovyan/Data/001.문서요약/"
...
for json_file in json_files:
    ...
    batched_args = chunkify(args_list, 500)

    # tqdm + 병렬처리 한번에 수행
    results_nested = process_map(
        process_sentence,
        batched_args,
        max_workers=4,
        chunksize=1,  # 작은 작업 단위로 설정 (너무 크면 초기 출력이 지연됨)
        desc="Processing sentences"
    )

    results = []
    for batch_result in results_nested:
        for sent_id, g2pk_result in batch_result.items():
            results.append({
                "id": sent_id,
                "g2p_result": g2pk_result
            })
